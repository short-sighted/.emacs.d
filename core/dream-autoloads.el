;; dream-autoloads.el --- Autoload cache infrastructure. -*- lexical-binding: t; -*-
;;
;;; Commentary:
;;
;; Overview / 概览
...
;;    `dream-autoloads-generate' writes `.local/var/borg/autoload/autoloads.el'
;;    and `.local/var/borg/autoload/autoloads-state.el'.
;;    全量重建或单包构建成功后，`dream-autoloads-generate' 写入聚合缓存和状态
;;    文件。
;;
;; Startup flow / 启动阶段流程
;; ---------------------------
;; 1. `init.el' adds Dream's own `core/' and `lisp/' directories to `load-path',
;;    then requires this library from `core/'.
;;    `init.el' 先把 Dream 自己的 `core/' 和 `lisp/' 加入 `load-path'，再从
;;    `core/' require 本库。
...
;; ---------------------------
;; - The generated file is source only: no byte compilation and no native
;;   compilation.
;;   生成文件只作为源码加载：不 byte-compile，也不 native-compile。
...
(defvar dream-local-directory (expand-file-name ".local" user-emacs-directory)
  "Dream's local data directory.
Dream 的本地数据目录。")
(defvar dream-var-directory (expand-file-name "var" dream-local-directory)
  "Dream's generated configuration-data directory.
Dream 放置生成型配置数据的目录。")
...
(defcustom dream-autoloads-directory
  (expand-file-name "borg/autoload/" dream-var-directory)
  "Directory where Dream's single autoload cache is written.
Dream 聚合 autoload 缓存的输出目录。"
  :type 'directory)

(defcustom dream-autoloads-file
  (expand-file-name "autoloads.el" dream-autoloads-directory)
  "File where Dream's single autoload cache is written.
Dream 启动时加载的聚合 autoload 源文件。"
  :type 'file)

(defcustom dream-autoloads-state-file
  (expand-file-name "autoloads-state.el" dream-autoloads-directory)
  "File where Dream records the inputs used to build the autoload cache.
记录生成缓存时使用的输入文件、目录和格式版本的状态文件。"
  :type 'file)

(defcustom dream-autoloads-borg-directory
  (expand-file-name "site-lisp/borg" user-emacs-directory)
  "Directory containing Borg itself.
This path is added only on the fallback path that has to require Borg during
startup.  The normal generated-autoload path does not load Borg.

Borg 自身所在目录。只有 fallback 需要 require Borg 时才会加入此路径；
正常的聚合 autoload 快速路径不会加载 Borg。"
  :type 'directory)
...
(defcustom dream-autoloads-stale-check-delay 2
  "Idle seconds before checking whether the loaded autoload cache is stale.
If nil, do not check staleness during startup.  If zero, check immediately.
The check never regenerates the cache; it only displays a warning.

加载缓存后等待多少 idle 秒再做过期检查。nil 表示启动期间不检查，0 表示
立即检查。该检查只提醒，不会重建缓存。"
  :type '(choice (const :tag "Do not check during startup" nil)
...
(defconst dream-autoloads--state-version 5
...
时只输出 Borg 增加的部分；没有基线时才退回到整值 `setq'，保证手工调用仍可用。"
  (let (forms)
    (dolist (entry values (nreverse forms))
      (let* ((var (car entry))
             (value (cdr entry))
...
(defun dream-autoloads--read-state ()
  "Read `dream-autoloads-state-file'.
Return nil when the file does not exist or cannot be read safely.

读取 `dream-autoloads-state-file'。文件不存在或内容损坏时返回 nil，让调用方
把缓存视为不可验证。"
...
(defun dream-autoloads--state-version-current-p ()
  "Return non-nil if the state file matches this cache format.
This is used by staleness checks only; startup should not fall back to Borg just
because the state version is old.

检查状态文件记录的缓存格式版本是否与当前代码匹配。该函数只用于过期检查；
启动不应仅因为状态版本旧就 fallback 到 Borg。"
  (when-let* ((state (dream-autoloads--read-state)))
    (equal (plist-get state :version) dream-autoloads--state-version)))

(defun dream-autoloads--state-current-p ()
  "Return non-nil if `dream-autoloads-state-file' matches current inputs.
This check is read-only and never regenerates the cache.

完整检查状态文件是否与当前输入匹配。该检查只读，不会重建缓存。"
  (condition-case nil
      (let ((state (dream-autoloads--read-state)))
        (and (equal (plist-get state :version) dream-autoloads--state-version)
             (cl-every #'dream-autoloads--entry-current-p
                       (plist-get state :files))
             (cl-every (lambda (entry)
                         (dream-autoloads--entry-current-p entry t))
                       (plist-get state :directories))))
    (error nil)))

(defun dream-autoloads--write-forms (file forms)
  "Write FORMS to autoload cache FILE.
The output is deterministic Lisp source that is loaded directly at startup and
is marked to avoid byte and native compilation.
...
(defun dream-autoloads--warn (message &rest args)
  "Display a Dream autoload warning using MESSAGE and ARGS.
Warnings are visible but do not interrupt startup.

显示 Dream autoload warning；提醒用户但不中断启动流程。"
  (display-warning 'dream-autoloads (apply #'format message args) :warning))

(defun dream-autoloads--fallback-to-borg (reason)
  "Fallback to `borg-initialize' after reporting REASON.
This is the slow but robust path for missing or unreadable autoload caches.

报告 REASON 后 fallback 到 `borg-initialize'。这是缓存缺失或无法加载时的
慢速但可靠路径。"
  (dream-autoloads--warn
   "%s Falling back to Borg initialization. Run `make build' or `make autoloads'."
   reason)
  (add-to-list 'load-path dream-autoloads-borg-directory)
  (require 'borg)
  (borg-initialize))

(defun dream-autoloads-check-state ()
  "Warn if the loaded Dream autoload cache is stale.
This function performs only file-attribute reads and never regenerates the
cache.  It is scheduled after startup by `dream-autoloads-initialize'.

检查已加载的聚合缓存是否过期。该函数只读取文件属性，不会重建缓存；通常
由 `dream-autoloads-initialize' 在启动后调度。"
  (unless (dream-autoloads--state-current-p)
    (dream-autoloads--warn
     "Dream autoload cache is stale. Startup used the existing cache; run `make build' or `make autoloads' soon.")))

(defun dream-autoloads--schedule-state-check ()
  "Schedule a non-mutating stale-cache check.
The delay is controlled by `dream-autoloads-stale-check-delay'.

按 `dream-autoloads-stale-check-delay' 调度只读过期检查，不做重建。"
...
(defun dream-autoloads--prepare-load ()
  "Prepare variables needed by generated cache forms.
Some optional startup variables, such as `Info-directory-list', may be unbound
before their library has loaded.  Package autoload caches can still add entries
to them, so bind the variable cheaply instead of requiring the library.

加载生成缓存前准备必要变量。例如 `Info-directory-list' 可能尚未由 info.el
定义；这里只做廉价绑定，不 require 额外库。"
  (unless (boundp 'Info-directory-list)
    (set 'Info-directory-list nil)))

(defun dream-autoloads-initialize ()
  "Load Dream's autoload cache for startup.
The fast path loads only the source `dream-autoloads-file'.  Borg is required
only if that cache is missing or fails to load.  Stale-cache detection is delayed
so it does not add synchronous work to startup.

启动时加载 Dream 聚合 autoload 缓存。正常快速路径只加载源码缓存，不 require
Borg；只有缓存缺失、读取失败或格式不兼容时才 fallback 到 Borg。"
  (let ((file dream-autoloads-file))
    (cond
     ((and noninteractive (featurep 'borg))
      ;; Borg build sessions already initialized package load paths.  Loading a
      ;; previously generated cache while byte-compiling init.el can reapply old
      ;; state, so batch builds intentionally skip the startup cache.
      t)
     ((file-readable-p file)
      (condition-case err
          (progn
            (dream-autoloads--prepare-load)
            (load file nil t)
            (dream-autoloads--schedule-state-check)
            t)
        (error
         (dream-autoloads--fallback-to-borg
          (format "Failed to load Dream autoload cache: %S." err)))))
     (t
      (dream-autoloads--fallback-to-borg
       "Dream autoload cache is missing.")))))
