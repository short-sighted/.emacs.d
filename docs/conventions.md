...
- `on` 表达一次性的编辑器生命周期事件。`on-first-input-hook` 启用
  `delete-selection-mode`，`on-init-ui-hook` 启动增量加载；它们不是包依赖协议。
...
`:also-load` 加载配套 feature。`once-setup` 提供 `:once`、`:require-once` 与 `:iload`；不要再造
`:after`、`:hooks`、`:init`、`:opt` 或 `:bind-map` 这类与 setup 上游语义重叠的别名。
